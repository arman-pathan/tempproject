...
class PropertyDetails extends Component {
  ...
  showEstimate = e => {
    ...
    var end = moment(
      moment(new Date(this.state.endDate))
        .add("days", 1)
        .format("YYYY-MM-DD")
    );
    var end2 = moment(new Date(this.state.endDate)).format("YYYY-MM-DD");
    ...
    if (noOfDays > 0 && noOfDays < 16) {
      ...
      while (!start.isSame(end)) {
        ...
        if (moment(new Date(start)).isBusinessDay()) {
      ...
      if (moment(new Date(end)).isBusinessDay()) {
        console.log("end: " + moment(new Date(end)));
        businessDays = businessDays + 1;
        console.log("businessDays: " + businessDays);
      } else {
        holidays = holidays + 1;
      ...
      // var businessDays = momentBus(start).businessDiff(momentBus(end));
      // if(businessDays == 0){
  ...
  handleBooking = e => {
    ...
    const requestBody = {
      ...
      ownerID: this.state.property.user.userID,
